...
//    $query = mysqli_query($con,$sql);
    $sql ="UPDATE image SET image.delete=0 WHERE image.imgID = '".$row3['imgID']."'";
    $query = mysqli_query($con,$sql);
}

//$sql2 = "DELETE FROM product WHERE product.pdID = '".$_GET['pdID']."'";
$sql2 ="UPDATE product SET product.delete=0 WHERE product.pdID = '".$_GET['pdID']."'";
//$query = mysqli_query($con,$sql2);
if ($con->query($sql2) === TRUE) {
    echo "<script type='text/javascript'>alert('แสดงข้อมูลเรียบร้อยแล้ว')</script>";
